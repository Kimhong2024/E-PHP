        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
            ...
            try {
                if ($_POST['action'] === 'add') {
                    ...
                    $stmt = $this->db->prepare("INSERT INTO customers (name, email, phone, address, password, status, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())");
                    $stmt->execute([$name, $email, $phone, $address, password_hash($password, PASSWORD_DEFAULT), $status]);
                    return ['success' => true, 'message' => 'Customer added successfully'];
                } elseif ($_POST['action'] === 'edit' && isset($_POST['id'])) {
                    // Update existing customer
                    if (!empty($password)) {
                        $stmt = $this->db->prepare("UPDATE customers SET name = ?, email = ?, phone = ?, address = ?, password = ?, status = ? WHERE id = ?");
                        $stmt->execute([$name, $email, $phone, $address, password_hash($password, PASSWORD_DEFAULT), $status, $id]);
                    } else {
                        $stmt = $this->db->prepare("UPDATE customers SET name = ?, email = ?, phone = ?, address = ?, status = ? WHERE id = ?");

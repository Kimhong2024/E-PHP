...
class Order {
    ...
    public function getProducts() {
        try {
            $stmt = $this->db->query("SELECT id, name, price, stock FROM products WHERE status = 'active' AND stock > 0 ORDER BY name");
  ...
  $('#addOrderItem').click(function() {
    ...
    products.forEach(product => {
      if (product && product.id && product.name && product.price) {
        ...
        productSelect.append(`<option value="${product.id}" data-price="${product.price || 0}" data-stock="${product.stock || 0}">
            ${product.name} - ${displayPrice} ${stockInfo}</option>`);
      }
    });
